use crate::db::{backup_models::{BackupData, RestoreBackupData}, token_store, DbState};
use rusqlite::params;
...
fn drive_token(state: &tauri::State<DbState>) -> Result<String, String> {
    token_store::get_valid_access_token(&state.vault, google_client_id(), google_client_secret())
}
...
pub fn upload_backup_to_drive(state: tauri::State<DbState>) -> Result<(), String> {
    ...
    let access_token = drive_token(&state)?;
    let conn = state.conn.lock().map_err(|e| e.to_string())?;
    ...
    if delay_ms > 0 {
        log::warn!("Retrying backup after {}ms, retry #{}", delay_ms, retry_count);
    ...
    let check_data: serde_json::Value = check_response.json()
    ...
    let upload_result = if let Some(files) = check_data["files"].as_array() {
        if let Some(existing) = files.first() {
            ...
            let update_response = client.patch(&format!(
        ...
        Err(format!("Failed to check files. Google API response: {}", check_data))
    };
...
pub fn restore_from_drive(state: tauri::State<DbState>) -> Result<String, String> {
    ...
    let download_response = client.get(&format!(
    ...
    let backup_content = download_response.text()
